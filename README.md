...
Fun fact: the reason for associating [_free()_](http://man7.org/linux/man-pages/man1/free.1.html) with [_munmap()_](https://linux.die.net/man/2/munmap) is that some allocations with [_malloc()/calloc()_](https://linux.die.net/man/3/malloc) will not use [_sbrk()_](https://linux.die.net/man/2/sbrk) and fall back to [_mmap()_](http://man7.org/linux/man-pages/man2/mmap.2.html) under the hood (with corresponding _munmap()_ to free memory).  
